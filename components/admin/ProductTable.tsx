...
export default function ProductTable() {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPrimaryIndex(0); // reset primary to first
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Append images: primary image FIRST, then rest
    ...
    // Also send primaryIndex for backend reference (optional)
    formData.append("primaryIndex", "0"); // after reorder, primary is always index 0
  ...
  const getCategoryProductCount = (categoryName: string) =>
    localProducts.filter((p) => p.category === categoryName).length;
          ...
          <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="space-y-3">
                <label className="text-sm font-medium text-gray-700">
                  ...
                  {imagePreviews.length > 0 && (
                    <span className="ml-2 text-xs text-gray-400">
                      — click a thumbnail to set as primary
                    </span>
                  )}
                </label>

                {/* Upload Zone */}
                <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center cursor-pointer hover:border-gray-400 hover:bg-gray-50 transition-all">
                ...
                {/* Image Previews Grid with Primary Selector */}
                {imagePreviews.length > 0 && (
                  <div className="grid grid-cols-4 gap-3 mt-2">
                    {imagePreviews.map((src, idx) => (
                      ...
                      >
                        ...
                        {/* Primary Badge */}
                        {idx === primaryIndex ? (
                          <div className="absolute top-1.5 left-1.5 bg-gray-900 text-white text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                            <Star size={10} fill="currentColor" />
                            Primary
                          </div>
                        ) : (
                          <div className="absolute top-1.5 left-1.5 bg-white/80 text-gray-600 text-[10px] px-2 py-0.5 rounded-full">
                            #{idx + 1}
                          </div>
                        )}
